...
const generateIndexHtml = (project: any) => {
  const page = project?.pages?.[0];
  if (!page) return "<!-- No page layout generated yet -->";
  ...
  <header class="hero-section">
    <div class="container">
      <h1>${sec.content?.heading || "Welcome to " + project.name}</h1>
      <p>${sec.content?.subheading || "Vision built instantly."}</p>
      <a href="${sec.content?.ctaUrl || '#'}" class="btn-primary">${sec.content?.ctaText || "Get Started"}</a>
    </div>
  </header>\n`;
  ...
  <section class="about-section">
    <div class="container">
      <h2>${sec.content?.heading || "About Us"}</h2>
      <p>${sec.content?.body || "Learn more about our services."}</p>
    </div>
  </section>\n`;
  ...
  <section class="contact-section">
    <div class="container">
      <h2>Get In Touch</h2>
      <p>Send an email to: <strong>${sec.content?.email || "support@example.com"}</strong></p>
    </div>
  </section>\n`;
    } else {
      sectionsHtml += `
  <!-- ${sec.type} Section -->
  <section class="${sec.type.toLowerCase()}-section">
    <div class="container">
      <h2>${sec.content?.title || sec.type}</h2>
      <p>Custom content block generated by AI.</p>
    </div>
  </section>\n`;
    }
  });
...
const generateGlobalCss = (project: any) => {
  const theme = project?.theme || {};
  const primaryColor = theme.primaryColor || "#3b82f6";
  const font = theme.fontFamily || "Inter, sans-serif";
  return `/* Global CSS Stylesheet for ${project.name} */
:root {
...
const generateMainJs = (project: any) => {
  return `/* Custom Interactive Script for ${project.name || "App"} */
document.addEventListener("DOMContentLoaded", () => {
  console.log("Website '${project.name || "App"}' initialized successfully.");
...
const highlightHtml = (code: string) => {
  return code
    ...
    .replace(/(&lt;\/?[a-zA-Z0-9:-]+)/g, '<span style="color: #f43f5e; font-weight: 600;">$1</span>')
    .replace(/(\s[a-zA-Z0-9:-]+=)/g, '<span style="color: #fbbf24;">$1</span>')
    .replace(/("[^"]*")/g, '<span style="color: #34d399;">$1</span>')
    .replace(/(&gt;)/g, '<span style="color: #f43f5e; font-weight: 600;">$1</span>');
};

const highlightCss = (code: string) => {
  return code
    .replace(/(\/\*[\s\S]*?\*\/)/g, '<span style="color: #6b7280; font-style: italic;">$1</span>')
    .replace(/([a-zA-Z0-9-]+:)/g, '<span style="color: #60a5fa;">$1</span>')
    .replace(/(#?[a-zA-Z0-9-]+;)/g, '<span style="color: #34d399;">$1</span>')
...
const highlightJs = (code: string) => {
  return code
    ...
    .replace(/(\bconst\b|\blet\b|\bvar\b|\bfunction\b|\breturn\b|\bdocument\b|\bwindow\b|\bconsole\b)/g, '<span style="color: #f43f5e; font-weight: 600;">$1</span>')
    .replace(/("[^"]*"|'[^']*')/g, '<span style="color: #34d399;">$1</span>')
...
export default function DashboardEditor({ user, tenant, baseDomain, protocol, initialPlans = [], upiId = "pogakula@ybl", initialSettings }: DashboardEditorProps) {
  ...
  const [activePreviewTab, setActivePreviewTab] = useState<"preview" | "inspect" | "code" | "settings">("preview");
  const [activeCodeFile, setActiveCodeFile] = useState<"index.html" | "global.css" | "main.js">("index.html");
    ...
    <div style={{ display: "flex", flexDirection: "column", width: "100%", height: "100vh", background: "var(--bg)", overflow: "hidden" }}>
      ...
      {isCreatingNew ? (
        ...
        {splitLayout !== "editor-only" && (
          ...
          <div style={{ padding: "0.5rem 1rem", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#0b0f19", flexWrap: "wrap", gap: "0.5rem" }}>
            
            {/* Left: Window Dots + Prompt Description + Quick Tools */}
            <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
              ...
              {currentProject && (
                <div style={{ display: "flex", flexDirection: "column", minWidth: 0 }}>
                  <span style={{ fontSize: "0.75rem", fontWeight: 700, color: "#fff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "200px" }}>
                    {currentProject.description || "Design a landing page..."}
                  </span>
                  <span style={{ fontSize: "0.6rem", color: "#6b7280", marginTop: "-0.1rem" }}>
                    Status: <span style={{ color: currentProject.status === "PUBLISHED" ? "#34d399" : "#a855f7" }}>{currentProject.status}</span>
                  </span>
                </div>
              )}
              ...
              <div style={{ display: "inline-flex", background: "rgba(255, 255, 255, 0.04)", border: "1px solid rgba(255, 255, 255, 0.05)", padding: "3px", borderRadius: "9999px", gap: "0.15rem" }}>
                {(["preview", "inspect", "code"] as const).map(tab => (
                  <button
                    ...
                    style={{
                      background: activePreviewTab === tab ? "#fff" : "transparent",
                      color: activePreviewTab === tab ? "#000" : "#94a3b8",
                      border: "none",
                      borderRadius: "9999px",
                      ...
                      boxShadow: activePreviewTab === tab ? "0 2px 6px rgba(0, 0, 0, 0.15)" : "none"
                    }}
                  >
            ...
            {currentProject && !isCreatingNew && (
              <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                <button
                  ...
                  title="Rebuild website layout"
                >
                  <Hammer size={11} className={currentProject.status === "GENERATING" ? "animate-spin" : ""} />
                  <span>Rebuild</span>
                </button>
                ...
                <button
                  onClick={() => {
                    // Trigger site publish modal or action
                    const publishBtn = document.querySelector('button[title="Publish to Live Subdomain"]') as HTMLButtonElement;
                    if (publishBtn) {
                      publishBtn.click();
                    } else {
                      window.open(activeSiteUrl, '_blank');
                    }
                  }}
                  style={{
                    ...
                    fontSize: "0.72rem",
                    fontWeight: 700,
                    boxShadow: "0 2px 8px rgba(32, 199, 181, 0.2)"
                  }}
                >
                  <Globe size={11} />
                  <span>Publish</span>
              ...
              currentProject.status === "GENERATING" || (currentProject.status === "DRAFT" && (!currentProject.pages || currentProject.pages.length === 0)) ? (
                ...
                activePreviewTab === "preview" ? (
                  ...
                  <div style={{ display: "flex", flexGrow: 1, gap: "1rem", overflow: "hidden", background: "#060a12", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "0.75rem", width: "100%" }}>
                    ...
                    <div style={{ width: "180px", borderRight: "1px solid rgba(255,255,255,0.06)", padding: "0.85rem", display: "flex", flexDirection: "column", gap: "0.8rem", userSelect: "none", flexShrink: 0, textAlign: "left" }}>
                      <span style={{ fontSize: "0.68rem", color: "#818cf8", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Workspace Files</span>
                      
                      <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.35rem", color: "#9ca3af", fontSize: "0.75rem", padding: "0.15rem 0" }}>
                          <span style={{ fontSize: "0.8rem" }}>📁</span> <strong>src</strong>
                        </div>
                        
                        {(["index.html", "global.css", "main.js"] as const).map(file => (
                          <div
                            key={file}
                            ...
                            style={{
                              ...
                              alignItems: "center",
                              gap: "0.35rem",
                              color: activeCodeFile === file ? "#fff" : "#9ca3af",
                              background: activeCodeFile === file ? "rgba(255,255,255,0.05)" : "transparent",
                              padding: "0.3rem 0.5rem",
                              borderRadius: "0.3rem",
                              fontSize: "0.72rem",
                              cursor: "pointer",
                              marginLeft: "0.7rem",
                              fontWeight: activeCodeFile === file ? 600 : 500,
                              transition: "all 0.1s"
                            }}
                          >
                            <span>{file.endsWith(".html") ? "📄" : file.endsWith(".css") ? "🎨" : "⚡"}</span>
                            {file}
                          </div>
                        ))}
                      </div>
                    ...
                    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "#07111b" }}>
                      <div style={{ background: "#0b111e", borderBottom: "1px solid rgba(255,255,255,0.05)", padding: "0.35rem 0.85rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: "0.68rem", color: "#fff", fontWeight: 600, fontFamily: "monospace" }}>{activeCodeFile}</span>
                        <span style={{ fontSize: "0.62rem", color: "#6b7280", fontFamily: "monospace" }}>UTF-8</span>
                      </div>
                      
                      <div style={{ flex: 1, overflow: "auto", padding: "0.85rem", fontFamily: "monospace", fontSize: "0.75rem", lineHeight: 1.5, background: "#07111b", textAlign: "left" }}>
                        <pre style={{ margin: 0, whiteSpace: "pre-wrap", wordBreak: "break-all" }}>
                          {activeCodeFile === "index.html" && (
                            <code dangerouslySetInnerHTML={{ __html: highlightHtml(generateIndexHtml(currentProject)) }} />
                          )}
                          {activeCodeFile === "global.css" && (
                            <code dangerouslySetInnerHTML={{ __html: highlightCss(generateGlobalCss(currentProject)) }} />
                          )}
                          {activeCodeFile === "main.js" && (
                            <code dangerouslySetInnerHTML={{ __html: highlightJs(generateMainJs(currentProject)) }} />
                          )}
                        </pre>
                      </div>
                    </div>
